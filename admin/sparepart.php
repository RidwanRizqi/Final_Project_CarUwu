...
<div class="konten">
    ...
    <div class="wrap">
        <div class="container-fluid">
            ...
            <button type="button" class="btn btn-success mb-3 mt-3" onclick="location.href='addSparepart.php'">
                Tambahkan Sparepart Baru
            </button>
            <table id="tabel-data" class="table table-striped table-bordered border-dark">
                ...
                <?php
                $i = 0;
                $sqlsparepart = "SELECT * FROM sparepart WHERE id_sparepart != '-'";
                $querysparepart = mysqli_query($koneksi, $sqlsparepart);
                while ($result = mysqli_fetch_array($querysparepart)) {
                    $idsparepart = $result['id_sparepart'];
                    $i++;
                    ?>
                    <tr align="center">
                        <td><?php echo $i; ?></td>
                        <td><?php echo $result['id_sparepart']; ?></td>
                        <td><?php echo $result['nama_sparepart']; ?></td>
                        <td><?php echo $result['tipe_sparepart']; ?></td>
                        <td><?php echo format_rupiah($result['harga_sparepart']) ?></td>
                        <td><?php echo $result['stok_sparepart'] ?></td>
                        <td>
                            <button type='button' class='btn btn-warning mb-2 edt-dlt'
                                    onclick="location.href='editSparepart.php?PRTID=<?php echo $idsparepart ?>'">
                                Edit
                            </button>
                            <button type='button' class='btn btn-danger mb-2 edt-dlt'
                                    onclick="location.href='deleteSparepart.php?PRTID=<?php echo $idsparepart ?>'">
                                Hapus
                            </button>
                        </td>
                    </tr>
